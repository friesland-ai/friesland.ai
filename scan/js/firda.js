/**
 * MBO AI-Geletterdheidsscan
 * Gebaseerd op het UNESCO AI Competency Framework for Teachers (2024)
 * Kleurenpalet: Basis #FFFFFF, Primair #3A5757, Secundair #95CDEA, Accent #FFCD00
 */
...
const UNESCO_ASPECTS = [
  {
    ...
    color: '#3A5757',
    badgeColor: 'bg-[#EBF3F3] text-[#2D4545] border-[#C4DADB] dark:bg-[#182828] dark:text-[#C4DADB] dark:border-[#3A5757]',
    icon: 'user-check',
  ...
  },
  {
    ...
    color: '#3A5757',
    badgeColor: 'bg-[#EEF7FC] text-[#235875] border-[#C2E3F4] dark:bg-[#132836] dark:text-[#95CDEA] dark:border-[#2C5F7C]',
    icon: 'shield-alert',
  ...
  },
  {
    ...
    color: '#3A5757',
    badgeColor: 'bg-[#FFFBEA] text-[#7A5B00] border-[#FCE8A2] dark:bg-[#2A2304] dark:text-[#FFCD00] dark:border-[#6B5000]',
    icon: 'sparkles',
...
const LIKERT_OPTIONS = [
  { 
    ...
    colorHex: '#bccec2',
    activeClass: 'bg-[#bccec2] text-[#1E2E2E] border-[#A3BCA9] shadow-md ring-2 ring-[#bccec2]/50',
    hoverBorder: 'hover:border-[#bccec2] hover:bg-[#bccec2]/15',
    activeSubtext: 'text-[#35483B]',
    activeNum: 'text-[#1E2E2E]'
  },
  { 
    value: 2, 
    ...
    colorHex: '#7e9e9b',
    activeClass: 'bg-[#7e9e9b] text-white border-[#678B88] shadow-md ring-2 ring-[#7e9e9b]/50',
    hoverBorder: 'hover:border-[#7e9e9b] hover:bg-[#7e9e9b]/15',
    activeSubtext: 'text-[#EAF5F4]',
  ...
  { 
    ...
    colorHex: '#ffcd00',
    activeClass: 'bg-[#ffcd00] text-[#2A2000] border-[#E5B800] shadow-md ring-2 ring-[#ffcd00]/50',
    hoverBorder: 'hover:border-[#ffcd00] hover:bg-[#ffcd00]/15',
    activeSubtext: 'text-[#5C4500]',
    activeNum: 'text-[#2A2000]'
  },
  { 
    value: 4, 
    ...
    colorHex: '#95CDEA',
    activeClass: 'bg-[#95CDEA] text-[#0E2736] border-[#74B8DB] shadow-md ring-2 ring-[#95CDEA]/50',
    hoverBorder: 'hover:border-[#95CDEA] hover:bg-[#95CDEA]/15',
    activeSubtext: 'text-[#174561]',
    activeNum: 'text-[#0E2736]'
  },
  { 
    value: 5, 
    ...
    colorHex: '#3A5757',
    activeClass: 'bg-[#3A5757] text-white border-[#2A4242] shadow-md ring-2 ring-[#3A5757]/50',
    hoverBorder: 'hover:border-[#3A5757] hover:bg-[#3A5757]/15',
    activeSubtext: 'text-[#D5E8E8]',
    activeNum: 'text-white'
  }
];
...
function calculateScores(answers) {
  ...
  UNESCO_ASPECTS.forEach(aspect => {
    ...
    let levelColor = 'bg-[#EBF3F3] text-[#2D4545] border border-[#C4DADB] dark:bg-[#182828] dark:text-[#C4DADB]';

    if (average >= 3.8) {
      unescoLevel = 'Niveau 3: Innoveren & Leiden (Create)';
      levelKey = 'create';
      levelBadge = 'Expert / Innovator';
      levelColor = 'bg-[#3A5757] text-white';
    } else if (average >= 2.5) {
      unescoLevel = 'Niveau 2: Toepassen / Verdiepen (Deepen)';
      levelKey = 'deepen';
      levelBadge = 'Gevorderd / Toepasser';
      levelColor = 'bg-[#95CDEA] text-[#162A36] font-bold';
    }
  ...
  let profile = {
    ...
    badgeClass: 'bg-[#3A5757] text-white',
    bgClass: 'bg-[#FAFDFD] border-[#E0EBEB] dark:bg-[#121D1D] dark:border-[#223636]',
  ...
  if (overallPercentage >= 80) {
    profile = {
      ...
      badgeClass: 'bg-[#3A5757] text-white',
      bgClass: 'bg-[#F2F8F8] border-[#BDD5D6] dark:bg-[#142323] dark:border-[#2E4747]',
      summary: 'Je bent een koploper in het MBO. Je zet AI niet alleen didactisch krachtig in voor jouw studenten en beroepsopdrachten, maar bewaakt scherp de ethiek, stimuleert menselijke regie en fungeert als inspirator en vraagbaak voor je team en de regio.'
    };
    ...
    profile = {
      ...
      badgeClass: 'bg-[#3A5757] text-white',
      bgClass: 'bg-[#F4F9FC] border-[#CDE5F3] dark:bg-[#12222D] dark:border-[#214256]',
    ...
    profile = {
      ...
      badgeClass: 'bg-[#3A5757] text-white',
      bgClass: 'bg-[#F7FAFA] border-[#D6E6E6] dark:bg-[#131F1F] dark:border-[#263D3D]',
      summary: 'Je experimenteert zo nu en dan met AI-tools en ziet de mogelijkheden voor het mbo. Er liggen mooie kansen om AI gerichter in te zetten voor werkdrukverlaging bij lesvoorbereiding, differentiëren naar mbo-niveaus en duidelijke afspraken over ethiek.'
    };
  }
...
function renderRadarChart(canvasId, aspectScores, isDarkMode = false) {
  ...
  const gridColor = isDarkMode ? 'rgba(149, 205, 234, 0.15)' : 'rgba(58, 87, 87, 0.18)';
  const angleLineColor = isDarkMode ? 'rgba(149, 205, 234, 0.20)' : 'rgba(58, 87, 87, 0.22)';
  const labelColor = isDarkMode ? '#E2EBEB' : '#223333';
  ...
  radarChartInstance = new Chart(ctx, {
      ...
      datasets: [
        {
          ...
          data: dataValues,
          backgroundColor: isDarkMode ? 'rgba(149, 205, 234, 0.28)' : 'rgba(58, 87, 87, 0.20)',
          borderColor: '#3A5757',
          borderWidth: 2.5,
          pointBackgroundColor: '#FFFFFF',
          pointBorderColor: '#3A5757',
          pointBorderWidth: 2,
          pointRadius: 5,
          pointHoverRadius: 7,
          pointHoverBackgroundColor: '#3A5757',
        ...
        },
        {
          ...
          borderColor: isDarkMode ? '#95CDEA' : '#95CDEA',
          borderWidth: 1.5,
          borderDash: [4, 4],
          pointRadius: 0,
          pointHoverRadius: 0,
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          backgroundColor: isDarkMode ? '#142222' : '#253838',
          titleColor: '#FFFFFF',
          bodyColor: '#E2EBEB',
          borderColor: '#3A5757',
          borderWidth: 1,
...
function updateChartTheme(isDarkMode) {
  if (radarChartInstance) {
    ...
    radarChartInstance.options.scales.r.ticks.color = isDarkMode ? '#88A3A3' : '#688282';
    radarChartInstance.options.plugins.legend.labels.color = labelColor;
    radarChartInstance.data.datasets[0].backgroundColor = isDarkMode ? 'rgba(149, 205, 234, 0.28)' : 'rgba(58, 87, 87, 0.20)';
      ...
      return `
        <button type="button" 
          ...
          class="flex-1 min-w-[170px] p-3 rounded-2xl border text-left transition-all duration-200 relative cursor-pointer
            ${isActive 
              ? 'bg-white dark:bg-[#192A2A] border-[#3A5757] shadow-sm ring-2 ring-[#3A5757]/20 text-[#1E2E2E] dark:text-white' 
              : 'bg-[#F5FAFA] dark:bg-[#121D1D] border-[#E2EBEB] dark:border-[#223535] text-[#557070] dark:text-[#8AA3A3] hover:border-[#95CDEA]'
            }">
          <div class="flex items-center justify-between mb-1.5 pointer-events-none">
            <span class="text-xs font-semibold uppercase tracking-wider ${isActive ? 'text-[#3A5757] dark:text-[#95CDEA]' : 'text-slate-500'}">
              Aspect ${asp.id}
            </span>
            ${isComplete 
              ? '<span class="inline-flex items-center text-[#3A5757] dark:text-[#95CDEA] text-xs font-bold"><i data-lucide="check-circle-2" class="w-3.5 h-3.5 mr-0.5"></i> Voltooid</span>' 
        ...
        <div class="question-card bg-white dark:bg-[#162424] rounded-2xl p-6 sm:p-8 border border-[#E0EBEB] dark:border-[#223636] shadow-sm relative" id="card-${q.id}">
          <div class="flex items-start justify-between gap-4 mb-4">
            <div class="flex items-center gap-2">
              <span class="flex items-center justify-center w-7 h-7 rounded-xl bg-[#EBF3F3] dark:bg-[#1C2E2E] text-[#3A5757] dark:text-[#95CDEA] font-bold text-xs border border-[#C4DADB] dark:border-[#3A5757]">
                ${q.number}
              </span>
            ...
            ${selectedVal !== undefined ? `
              <span class="inline-flex items-center gap-1 text-xs font-bold text-[#3A5757] dark:text-[#95CDEA] bg-[#EBF3F3] dark:bg-[#1C2E2E] px-2.5 py-1 rounded-full border border-[#C4DADB] dark:border-[#3A5757]">
          ...
          <div class="mb-5">
            <button type="button" 
              ...
              class="inline-flex items-center gap-2 text-xs font-semibold text-[#3A5757] dark:text-[#95CDEA] bg-[#F5FAFA] dark:bg-[#162626] hover:bg-[#EBF3F3] dark:hover:bg-[#1E3333] border border-[#DCE8E8] dark:border-[#273D3D] px-3.5 py-1.5 rounded-xl transition cursor-pointer">
              <i data-lucide="info" class="w-3.5 h-3.5 text-[#3A5757] dark:text-[#95CDEA] pointer-events-none"></i>
            ...
            <div id="example-box-${q.id}" class="hidden mt-2.5 bg-[#FAFDFD] dark:bg-[#111C1C] rounded-xl border border-[#E5EEEE] dark:border-[#1E3030] p-4 text-xs sm:text-sm">
              <div class="flex items-start gap-2.5 text-[#3A5757] dark:text-[#C3DCDC]">
                <i data-lucide="sparkles" class="w-4 h-4 text-[#FFCD00] shrink-0 mt-0.5"></i>
                <div>
        ...
        <div class="flex items-center gap-3">
          ${allAnswered ? `
            <button type="button" 
              ...
              class="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-[#3A5757] hover:bg-[#2C4444] text-white font-bold text-sm shadow-md transition scale-105 cursor-pointer">
          ...
          ` : isLastAspect ? `
            <button type="button" 
              data-nav-action="view-results"
              class="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-[#3A5757] text-white font-bold text-sm shadow-md hover:bg-[#2C4444] transition cursor-pointer">
          ...
          ` : `
            <button type="button" 
              ...
              class="inline-flex items-center gap-2 px-6 py-2.5 rounded-xl bg-[#3A5757] text-white font-bold text-sm shadow-sm hover:bg-[#2C4444] transition cursor-pointer">
...
function renderResultsView() {
        ...
        <div class="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 relative z-10">
          <div class="space-y-3 max-w-2xl">
            <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${scores.profile.badgeClass}">
              <i data-lucide="award" class="w-3.5 h-3.5 text-[#FFCD00]"></i>
            ...
            <div class="pt-1 flex flex-wrap items-center gap-4 text-xs font-medium text-[#557070] dark:text-[#8AA3A3]">
              <span class="inline-flex items-center gap-1.5 bg-white dark:bg-[#182828] px-3 py-1.5 rounded-lg border border-[#E0EBEB] dark:border-[#263D3D]">
                <i data-lucide="book-open" class="w-3.5 h-3.5 text-[#3A5757] dark:text-[#95CDEA]"></i>
          ...
          <div class="flex flex-row lg:flex-col items-center justify-center p-6 bg-white dark:bg-[#162424] rounded-2xl border border-[#E0EBEB] dark:border-[#223636] shadow-sm shrink-0 w-full lg:w-48 text-center">
            <div class="text-4xl sm:text-5xl font-black text-[#3A5757] dark:text-[#95CDEA]">
  ...
  if (aspectGridEl) {
    aspectGridEl.innerHTML = scores.aspectScores.map(asp => {
      ...
      return `
        <div class="bg-white dark:bg-[#162424] rounded-3xl p-6 sm:p-7 border border-[#E0EBEB] dark:border-[#223636] shadow-sm hover:shadow-md transition">
          ...
          <div class="space-y-1.5 mb-5">
            <div class="flex items-center justify-between text-xs font-bold text-[#445E5E] dark:text-[#C4DADB]">
              ...
              <span class="text-[#3A5757] dark:text-[#95CDEA] font-extrabold">${asp.percentage}%</span>
            </div>
            <div class="w-full h-2.5 bg-[#E8F0F0] dark:bg-[#1F3333] rounded-full overflow-hidden">
              <div class="h-full bg-gradient-to-r from-[#3A5757] to-[#95CDEA] rounded-full" style="width: ${asp.percentage}%;"></div>
            </div>
          </div>
          ...
          <div class="bg-[#f8fafc] dark:bg-[#0b111e] rounded-2xl p-4 border border-slate-200 dark:border-slate-800 text-xs sm:text-sm">
            <div class="font-bold text-slate-900 dark:text-white mb-2 flex items-center gap-1.5">
              <i data-lucide="check-circle" class="w-4 h-4 text-[#3A5757] dark:text-[#95CDEA]"></i>
...
function handleGlobalClicks(e) {
  ...
  if (copySumBtn) {
    ...
    const summaryText = `🎓 MBO AI-Geletterdheidsscan Resultaat (UNESCO AI CFT Kader) (UNESCO AI CFT Kader)
--------------------------------------------------
...
Gemaakt met de MBO AI-Geletterdheidsscan.`;

    navigator.clipboard.writeText(summaryText).then(() => {
      const originalHTML = copySumBtn.innerHTML;
      copySumBtn.innerHTML = '<i data-lucide="check" class="w-4 h-4 text-[#FFCD00]"></i> <span>Gekopieerd!</span>';
      safeIcons();
      setTimeout(() => {
